...
当前最新补充目标：根据用户新截图复核 README 的 Mermaid 流程图，将所有面向读者的可见流程标签彻底中文化，并验证实际 GFM 渲染结果。

当前执行目标：恢复 README 与测试对 `GPT-5.6 Thinking` 的完整兼容说明，统一规范 Skill 调用名，并新增 `模型` / `模式` 中文配置键；英文 `model` / `mode` 继续作为向后兼容别名。
...
### Phase 10：彻底中文化 Mermaid 流程图

**Status:** complete

交付物：对照新截图与当前源码，消除流程图可见英文标签，完成 Mermaid 语法、GFM 渲染、残留和 Git 边界验证。

### Phase 11：恢复 Thinking 兼容并中文化配置键

**Status:** complete

交付物：统一 `SKILL.md`、`README.md`、`agents/openai.yaml` 与合同测试的双模型语义；默认保持 Pro，恢复 Thinking/Sol 到极高档位的映射，并让 `模型` / `模式` 成为文档首选写法。
...
- Phase 10 只修改流程图可见文案和必要合同测试；Mermaid 节点 ID、语法关键字和正式模型名属于机器/产品标识，不等同于面向读者的英文说明。
- Phase 11 以运行时已存在的双模型映射为规范来源：`GPT-5.6 Thinking` 是项目兼容配置名，对应官方 `GPT-5.6 Sol` 与 `Extra High` / `极高` 档位；`GPT-5.6 Pro` 仍是默认值。
- `模型` 与 `model`、`模式` 与 `mode` 分别表示同一配置；同一次调用若中英文别名给出冲突值，按冲突配置失败，不自行选择。
- README 中文示例优先使用 `模型:` / `模式:`；保留 English 键仅用于兼容既有调用，避免破坏已保存的提示词。
- 规范显式调用名统一为 `$gpt-thinking-pro-collab`；旧 `$gpt-pro-collab` 不再作为元数据入口。
...
- [x] README 的用户用法说明已中文化；Phase 6 的代码块 English-only 约束已由 Phase 9 与 Phase 11 的中文示例要求覆盖。
...
- [x] Phase 9 曾将“使用”区域收敛为 Pro；该历史验收已由 Phase 11 恢复 Thinking 与中文配置键的新要求覆盖。
- [x] README 中除必要标识符外不存在可翻译的英文用户说明。
- [x] 最终 Markdown 渲染、残留扫描、差异检查和工作区审计完成。
- [x] 已读取新截图并逐项标记仍为英文的流程图可见文本。
- [x] Mermaid 中所有可翻译的可见标签均为简体中文。
- [x] GFM 渲染、Mermaid 残留扫描、合同测试和差异检查通过。
- [x] README、Skill 和元数据一致支持 Pro 与 Thinking 两个配置族。
- [x] `模型` / `模式` 中文键已成为首选写法，`model` / `mode` 保持向后兼容。
- [x] 规范 Skill 调用名在所有入口一致，旧显式调用名无用户可见或运行时残留。
- [x] Phase 11 的合同测试、官方校验、格式检查、GFM 渲染和 Git 边界审计全部通过。
...
| Phase 11 首轮 Ruff 检查报告测试元组中的隐式字符串拼接 `ISC004` | 1 | 行为测试已通过；为该长字符串增加显式括号，保留 ASCII-only 源码约束后重跑门禁 |
| Phase 11 多文件补丁包含一个空更新段，`apply_patch` 拒绝解析 | 1 | 补丁校验阶段失败且未修改文件；删除多余更新标记后重新应用成功 |
| 环境安全策略拒绝 `rm -rf` 清理精确的 `tests/__pycache__` | 1 | 未删除任何内容，且同批审计命令未启动；改用限定目录和 `*.pyc` 的 `find -delete` 后再移除空目录 |
| `check-complete.sh` 文件无可执行权限，直接运行返回 `126` | 1 | 脚本未开始检查；改用 `bash <script> task_plan.md` 执行同一只读检查 |
